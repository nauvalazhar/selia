...
.dark {
  ...
  --secondary: oklch(0.3564 0.0076 264.47);
  --secondary-foreground: var(--foreground);
  ...
  --info-border: oklch(0.5703 0.160671 249.8244);
  --kbd: oklch(0.4349 0.0085 248.02);
  --kbd-foreground: var(--foreground);
  --kbd-border: oklch(0.5076 0.0103 248.03);
}
```
...
Another example where the `render` prop is useful is when you want to wrap a component with another component.
For example, you can wrap a `DialogTrigger` with a `Tooltip` component.

```tsx filename="Dialog Trigger with Tooltip"
<Dialog>
  <Tooltip>
    <TooltipTrigger 
      render={
