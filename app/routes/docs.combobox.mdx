# Combobox

Combined input and menu for selecting or entering values with autocomplete.

<DocsButtons externalDoc="https://base-ui.com/react/components/combobox" />

<Preview name="basic" />

## Installation
<Installation name="combobox" dependencies={["chip"]} />

## Usage

### Import

Import the component and its related parts.

```ts filename="Combobox Parts"
import {
  Combobox,
  ComboboxPopup,
  ComboboxEmpty,
  ComboboxItem,
  ComboboxList,
  ComboboxSearch,
  ComboboxTrigger,
  ComboboxValue,
} from 'components/selia/combobox';
```

### Structure

The expected structure and composition of the component.

```tsx filename="Anatomy"
<Combobox items={items} defaultValue={items[0]}>
  <ComboboxTrigger>
    <ComboboxValue />
  </ComboboxTrigger>
  <ComboboxPopup>
    <ComboboxSearch />
    <ComboboxEmpty>No results found</ComboboxEmpty>
    <ComboboxList>
      {(item) => (
        <ComboboxItem key={item.value} value={item}>
          {item.label}
        </ComboboxItem>
      )}
    </ComboboxList>
  </ComboboxPopup>
</Combobox>
```

The `items` prop is an array of objects that define the options to be displayed in the autocomplete popup. Each object should have a `value` and a `label` property.

```ts filename="items.ts"
const items = [
  { value: 'software-engineering', label: 'Software Engineering' },
  { value: 'machine-learning', label: 'Machine Learning' },
  { value: 'data-science', label: 'Data Science' },
  { value: 'artificial-intelligence', label: 'Artificial Intelligence' },
];
```

### Props

Each component part, its responsibility, and the props it exposes.

<ComponentTable
  components={{
    ComboboxTrigger: {
      source: {
        label: 'Base UI Combobox',
        href: 'https://base-ui.com/react/components/combobox'
      },
      required: true,
      props: {
        variant: {
          options: ['default', 'subtle', 'plain'],
          default: 'default',
        },
        pill: {
          options: ['true', 'false'],
        },
      },
    },
  }}
/>

## Examples

### Variants

Combobox comes in a variety of variants, each with its own color and style.
You can choose between a `subtle` or `plain` variant.

<Preview name="variant" />
### Multiple Select

Allow users to select multiple options from a set of choices with `Combobox` component.

<Preview name="multiple" />

### Group

You can group the items in the combobox using the `ComboboxGroup`, `ComboboxGroupLabel`, and `ComboboxCollection` components.

<Preview name="group" />
